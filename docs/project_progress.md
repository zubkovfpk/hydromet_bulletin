...
| DT-13-3 | **[OPEN / HIGH — сессия 14, critical path]** **Title:** date policy не учитывает GFS. **Symptom:** `_resolve_run_date_for_dry_run` выбирает `run_date = today` даже когда GFS-файлов за today в storage нет, а за today-1 — есть; pipeline падает на стадии `collect_meteo_data` до `collect_wave_data`. **Evidence (dry-run 20260420 без --date):** `Dry-run run_date=today=20260420 (UTC)`; `GFS storage dir not found: data\storage\gfs\20260420\00z`; `FileNotFoundError: No GFS .nc files found for run_date=20260420, cycle=00z`. **Root cause:** логика проверки привязана только к CMEMS; GFS availability не анализируется. **Proposed fix:** симметричная проверка GFS в `_resolve_run_date_for_dry_run`; fallback today-1 применяется, если GFS за today отсутствует; при конфликте CMEMS/GFS — выбирается пересечение или контролируемая ошибка. **DoD:** dry-run без `--date` завершается успешно при наличии GFS за today-1 и CMEMS за today/today-1; pipeline проходит до `collect_wave_data` без `FileNotFoundError` по GFS. **Related:** DT-13-2 (closed by scope), DT-13-4 (blocks full E2E совместно). | **high** | Сессия 14 |
| DT-13-4 | **[OPEN / HIGH — сессия 14, critical path, clean migration]** **Title:** storage layout mismatch — `collect_meteo_data` дефолт `results_subdir` указывает в legacy-каталог. **Symptom:** `collect_meteo_data.results_subdir = "Meteo_Parser_2026/results"` как default; `forecast_morning.py` не переопределяет; поиск `.nc` уходит в legacy-каталог даже при корректном `run_date`. **Evidence:** legacy-путь присутствует только в `utils/collect_meteo_data.py` и `tests/test_processing_layout_paths.py`; реальные `.nc` ожидаются в `data/storage/gfs/<date>/<cycle>/`. **Root cause:** техдолг сессии 7 (legacy MATLAB-layout как дефолт); clean migration не выполнялась. **Proposed fix (clean migration, решение пользователя):** поменять дефолт `results_subdir` в `collect_meteo_data` на новый layout; обновить `tests/test_processing_layout_paths.py` (xfail или delete legacy-тесты); при необходимости минимально поправить callers (`forecast_morning.py`, `forecast_evening.py`). **DT-13-5 не заводится** — поглощён данной задачей. **DoD:** pipeline ищет `.nc` в новом layout без переопределения `results_subdir` в callers; legacy-тесты обработаны согласованно. **Related:** DT-13-3 (blocks full E2E совместно). | **high** | Сессия 14 |
| DT-13-6 | **[OPEN / LOW — sweep-сессия]** **Title:** config drift — `files_per_cycle` 5 vs 10. **Symptom:** `config.ini` `[CMEMS_FORECAST]` `files_per_cycle = 5`, `config.example.ini` `files_per_cycle = 10`. **Proposed fix:** согласовать в обоих файлах; решение о канонической цифре — при выполнении sweep. | **low** | Sweep |
