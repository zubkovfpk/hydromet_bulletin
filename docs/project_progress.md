...
## Сессия 13 — план (шаги 13.A + 13.A.2: docs pre-work выполнен)

**Критический путь:**
- DT-12-2 Part 2: `validate_outputs._validate_dates` → сравнение в часах с `forecast_hours` (Вариант 2, переписывание семантики).
- DT-13-1: миграция `forecast_days` → `forecast_hours` (compat-window β, CLI-флаг, config согласованы).
- DT-13-2: dry-run date policy enforcement (today / fallback today-1).

**Статус DT-12-2:**
- Part 1 ✅ закрыт локально (fffc359): min/max по union(time_arr) в `collect_wave_data`.
- Part 2 ⚠️ open (13.B): fix `_validate_dates` в `validate_outputs.py`.

**Примечание по вариантам:**
- Вариант 1 → Part 1 DT-12-2 закрыт (fffc359).
- Вариант 2 → основной путь Part 2 DT-12-2 в 13.B (переписывание `_validate_dates`, не «калибровка»).
- Вариант 3 → **rescinded (13.A.2):** CMEMS нормативно возвращает штатный набор из 4 отсечек × 12 ч × 2 файла/сутки, перекрытий нет, «догрузка» неприменима.

**Dry-run policy:** основная дата = today (UTC); fallback = today-1. Дата 20260415 зафиксирована в 13.A по ошибке; результаты dry-run 13.B на 20260415 признаются НЕ верификационными.

**Stretch-goal:** генерация `.docx` на dry-run today / today-1.

**Git policy сессии 13 (подтверждена на шаге 13.A.2):**
- Cursor коммитит код ЛОКАЛЬНО поверх `cc96416` и `fffc359`, НЕ пушит.
- Windsurf в шаге 13.A.2 делает docs-коммит и пушит сразу.
...
**DoD сессии 13:** см. DT-12-2 пункты (a)–(g).
