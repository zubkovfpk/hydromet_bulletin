...
- **Каноническая ориентация осей для wave-массивов** (сессия 12): по аналогии с meteo-слоем целевая каноника `collect_wave_data` — **`(lat, lon, n_days)`** = lat-first, NetCDF-стандарт. Текущая реализация (до DT-12-1) использует `(lon, lat, n_days)` — MATLAB-легаси: `np.transpose(h_wave.data, (2,1,0))` даёт `(lon, lat, time)`, `H_Wave = np.zeros((nx_full, ny_full, ...))` где `nx_full=len(lon_full)`, meshgrid с `.T`. Аналог Blocker #4 / DT-10-3. DT-12-1 должен привести wave-слой к единой каноники с meteo-слоем (Вариант A: транспозиция `(1,2,0)`, `H_Wave(ny_full, nx_full, 40)`, `np.ix_(lat_mask, lon_mask, ...)`, убрать `.T`).
...
- **DT-11-1** (сессия 12, частично адресован Cursor, acbcfba): CMEMS `.nc` not found в `collect_wave_data`. Cursor реализовал 3-tier discovery (флэт → nested → legacy). DoD: dry-run проходит `collect_wave_data` без `FileNotFoundError`. Приоритет: high. Этап: сессия 12.
- **DT-12-1** ⚠️ **(сессия 12)**: `collect_wave_data.py` возвращает `Wave` в `(lon, lat, 5)` — MATLAB-легаси, нарушает канонику. Целевая каноника: `(lat, lon, 5)`, аналогично `collect_meteo_data`. Вариант A (рек.): заменить транспозицию `(2,1,0)` → `(1,2,0)`, `H_Wave(ny_full, nx_full, 40)`, `np.ix_(lat_mask, lon_mask, ...)`, убрать `.T` в meshgrid. Приоритет: high. Этап: сессия 12.
...
- **DT-11-1** (сессия 12): CMEMS `.nc` not found — 3-tier discovery реализован Cursor (acbcfba). DoD: dry-run проходит `collect_wave_data` без ошибки. Приоритет: high.
- **DT-12-1** ⚠️ **Blocker #6 (сессия 12)**: `collect_wave_data` возвращает `(lon, lat, 5)` вместо `(lat, lon, 5)` — MATLAB-легаси `.T`. Аналог DT-10-3. Должно быть устранено по Варианту A. Приоритет: high.
- **Normalizing/preprocessing layer для GFS**: после v1, если прямой переход `gfs_downloader` → `collect_meteo_data` останется неудобным.
