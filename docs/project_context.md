...
- **Временно́й контракт wave-данных в collect_wave_data — Part 1 закрыт (сессия 13, DT-12-2, fffc359 local)**: `start_date` и `end_date`, возвращаемые `collect_wave_data`, вычисляются как `min` и `max` по объединённому массиву всех `time_arr` из всех `.nc`-файлов, найденных 3-tier discovery (DT-11-1). **Нормативная формула Part 1:** `time_arr_all = concat(time_arr_f0, ..., time_arr_fN)`; `start_date = _hours_to_date(min(time_arr_all))`; `end_date = _hours_to_date(max(time_arr_all))`. **Part 1 реализован в fffc359 (локальный коммит, не запушен)**. **Part 2 — в `validate_outputs._validate_dates`:** уйти от `.days`; сравнивать `(end − start).total_seconds() / 3600` с `forecast_hours` из конфига; конкретная формула допуска уточняется при реализации в 13.B. Ось wave-массива `(n_lat, n_lon, n_days)` (DT-12-1) не затрагивается. 3-tier discovery DT-11-1 не меняется.
...
- **Параметр горизонта прогноза forecast_hours** (сессия 13, DT-13-1, нормативный): единственный источник правды для объёма запроса CMEMS и для валидации горизонта — ключ `forecast_hours` в секции `[CMEMS_FORECAST]` конфига (единица: часы; дефолт 120 = 5 суток × 24 ч). Compat-window β: одновременно поддерживаются `forecast_hours` (новый) и `forecast_days` (deprecated); при отсутствии `forecast_hours` — fallback `forecast_days × 24` с deprecation warning в логах; удаление `forecast_days` — в sweep-сессии. CLI-override: флаг `--forecast-hours` у `forecast_morning.py` и `forecast_evening.py`; CLI перекрывает config. Текущее рассогласование `config.ini=5` vs `config.example.ini=10` приводится к согласованному значению 120 в обоих файлах как часть DT-13-1 в 13.B.
- **Validation horizon semantics** (сессия 13, DT-12-2 Part 2, нормативный): `validate_outputs._validate_dates` **обязан** сравнивать горизонт в часах: `(end_date − start_date).total_seconds() / 3600` vs `forecast_hours` из конфига. Использование `.days` **запрещено**: даёт false positive «0 day(s)» на нормативных CMEMS-данных, где `end − start` < 1 суток. Конкретная формула допуска (`==`, `>=`, `± N ч`) уточняется при реализации в 13.B.
...
- **DT-12-2** ⚠️ **IN PROGRESS (сессия 13)**: wave temporal validation after DT-12-1 fix. Part 1 закрыт (fffc359 local): min/max по union(time_arr). Part 2 open (13.B): `_validate_dates` уйти от `.days`, сравнение в часах с `forecast_hours`. Вариант 3 rescinded (CMEMS нормативный набор штатный). Приоритет: high.
- **DT-13-1** ⚠️ **ready-for-implementation (сессия 13.B)**: `forecast_days` → `forecast_hours` migration (compat-window β). `forecast_hours` в `[CMEMS_FORECAST]`, дефолт 120; compat `forecast_days × 24` с deprecation warning; CLI `--forecast-hours`; `config.ini`/`config.example.ini` → 120. Удаление `forecast_days` — sweep-сессия. Приоритет: medium.
- **DT-13-2** ⚠️ **ready-for-implementation (сессия 13.B)**: dry-run date policy enforcement. Дефолт `--date` = today (UTC); fallback today-1 с логом. Сохранить явный `--date YYYYMMDD`. Related: DT-12-2 Part 2. Приоритет: medium.
...
- **DT-12-2** ⚠️ **IN PROGRESS (сессия 13)**: wave temporal validation. Part 1 закрыт (fffc359 local): min/max по union(time_arr). Part 2 open: `_validate_dates` → часы vs `forecast_hours`. Вариант 3 rescinded. Приор: high.
- **DT-13-1** ⚠️ **ready-for-impl (13.B)**: `forecast_hours` в `[CMEMS_FORECAST]`, дефолт 120; compat-window β (`forecast_days` deprecated); CLI `--forecast-hours`; config согласованы. Приор: medium.
- **DT-13-2** ⚠️ **ready-for-impl (13.B)**: dry-run date policy — дефолт today (UTC), fallback today-1. Приор: medium.
- **Normalizing/preprocessing layer для GFS**: после v1, если прямой переход `gfs_downloader` → `collect_meteo_data` останется неудобным.
