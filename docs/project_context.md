...
- **Временно́й контракт wave-данных в collect_wave_data** (сессия 13, DT-12-2, Вариант 1): `start_date` и `end_date`, возвращаемые `collect_wave_data`, **обязаны** вычисляться как `min` и `max` по объединённому массиву всех `time_arr` из всех `.nc`-файлов, найденных 3-tier discovery (DT-11-1, acbcfba). Запрещено брать `time_arr[0]` только из первого или последнего файла: при наборе из 2 CMEMS cycle-файлов (00 и 12 одной даты) такой подход даёт Δdays = 0 и `TemporalValidationError "Wave horizon looks suspicious: 0 day(s)"`. **Нормативная формула:** `time_arr_all = concat(time_arr_f0, ..., time_arr_fN)`; `start_date = _hours_to_date(min(time_arr_all))`; `end_date = _hours_to_date(max(time_arr_all))`; Δdays = `(end_date − start_date).days`; Δdays должен быть согласован с `validate_outputs._validate_dates` **без изменения самого `validate_outputs`** (только корректировка `collect_wave_data`). Ось wave-массива остаётся `(n_lat, n_lon, n_days)` по каноникe DT-12-1 — данное правило её не меняет. 3-tier discovery DT-11-1 не меняется. DoD DT-12-2: `TemporalValidationError` исчезает на dry-run 20260415; `collect_wave_data` возвращает `start_date` и `end_date` корректными.
...
- **DT-12-2** ⚠️ **критический путь сессии 13**: `validate_outputs.assert_valid_for_bulletin` `TemporalValidationError` (wave horizon 0 days) + NaN ~99.98%. Root cause (confirmed): `start_date`/`end_date` берутся как `time_arr[0]` из 2 файлов (00+12) → Дdays=0. Не связан с DT-12-1. Приоритет: high.
...
- **DT-12-2** ⚠️ **критический путь сессии 13**: `validate_outputs` `TemporalValidationError` (horizon 0 days) + NaN ~99.98%. Root cause (confirmed): `time_arr[0]` из 2 файлов (00+12) → Дdays=0. Не связан с DT-12-1. Приор: high.
- **Normalizing/preprocessing layer для GFS**: после v1, если прямой переход `gfs_downloader` → `collect_meteo_data` останется неудобным.
