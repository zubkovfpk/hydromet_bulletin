...
> **Временное исключение (S17–S19, ADR-003):** до введения
> внешнего оркестратора (S20+ HTTP API) forecast_main.py
> допускает локальный auto-trigger ingest_gfs.py через subprocess
> при on-demand запросе и устаревшем manifest. Это осознанный
> trade-off практичности над чистотой разделения слоёв.
> Целевая архитектура с полным разделением восстанавливается
> в S20+. Детали — ADR-003.
