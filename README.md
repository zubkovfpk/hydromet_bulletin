...
> ℹ️ **Демо-статус:** репозиторий опубликован как демонстрация hands-on компетенций в архитектуре и разработке AI-driven решений. Источники данных и учётные данные, используемые в проекте, могут быть тестовыми и неактуальными; проект не предназначен для принятия оперативных решений без самостоятельной проверки результатов.
...
`telegram_bot/` — git submodule ([Beavisinc/bulletin_tg_bot](https://github.com/Beavisinc/bulletin_tg_bot)). Telegram-бот на aiogram + Flask API, генерирует бюллетень по точке (lat/lon) по запросу пользователя, в отличие от основного конвейера этого репозитория (ежедневный email-бюллетень по всей акватории).

**Авторство:** код разработан сотрудником команды VIZARD как служебное произведение в рамках платформы под моим руководством; архитектура интерфейса, постановка задачи и решение об интеграции в общий сервис — мои.
