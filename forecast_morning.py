"""
forecast_morning.py
Утренний гидрометеорологический бюллетень (09:00 MSK).
Формирует прогноз на 5 суток, сохраняет .docx и отправляет на email.

Запуск:
    python forecast_morning.py [--date YYYYMMDD] [--outdir ./output] [--config config.ini]
"""
...
def run_morning(cfg: configparser.ConfigParser,
    ...
    """
    Основная функция утреннего бюллетеня.
    ...
    try:
        ...
        _converted = _gfs_dl.convert_existing(date=effective_run_date, cycle=gfs_cycle)
        ...
        for n in range(len(dtime) - 1):
            ...
            period_label = f"С 9:00 {d1} до 9:00 {d2}"
            ...
            temperature = temp_statistics_morning(
        ...
        header_date  = f"от {start_str} 09:00"

        Path(out_dir).mkdir(parents=True, exist_ok=True)
        filename = f"Прогноз_утро_{start_date.strftime('%Y%m%d')}.docx"
        out_path = str(Path(out_dir) / filename)

        create_bulletin_doc(
            output_path=out_path,
            bulletin_type="morning",
...
if __name__ == "__main__":
    ...
    warnings.warn(
        "forecast_morning.py is deprecated (ADR-001 Scenario X). "
        "Use 'python forecast_main.py --date <YYYY-MM-DD> --time <HH:MM> --tz MSK'. "
        "Hard-cut planned for S18.",
        DeprecationWarning,
        stacklevel=2,
    )
    parser = argparse.ArgumentParser(description="Утренний метеобюллетень")
    ...
    run_morning(cfg=cfg, run_date=args.date, forecast_hours_override=args.forecast_hours)
